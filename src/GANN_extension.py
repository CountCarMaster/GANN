...
from src.utils import keep_feature_extension
...
class DGCNN_cls(nn.Module):
    ...
    def forward(self, x): # [B, C, N]
        B, C, N = x.shape
        x = keep_feature_extension(x, self.k) # [B, C, k, N]
        x = self.edge_cov1(x)
        x = torch.max(x, dim=2)[0]

        x = keep_feature_extension(x, self.k)
        x = self.edge_cov2(x)
        x1 = torch.max(x, dim=2)[0]

        x = keep_feature_extension(x1, self.k)
        x = self.edge_cov3(x)
        x = torch.max(x, dim=2)[0]
